...
const { handler } = require('../../../src/routes/empresa/src/delete');
const EmpresaModel = require('../../../src/models/Empresa');

describe('Delete Empresa api test', () => {
	...
	context('When no error occurs', () => {
		it('Should 200 if successfully delete', async () => {
			sandbox.stub(EmpresaModel.prototype, 'eliminar').resolves(falsoId);
			...
			assert.deepStrictEqual(res.json, { message: 'Se eliminio la empresa correctamente' });

			sandbox.assert.calledOnceWithExactly(EmpresaModel.prototype.eliminar, body.id, 'empresa');
		});
	});
	...
	context('When error occurs', () => {
		context('When error in validate', () => {
			it('Should 400 if body wrong', async () => {
				...
				sandbox.assert.notCalled(EmpresaModel.prototype.eliminar);
			});

			context('When error in program', () => {
				it('Should return 500 if method insert fails', async () => {
					sandbox.stub(EmpresaModel.prototype, 'eliminar').rejects(new Error('Error in Delete'));
